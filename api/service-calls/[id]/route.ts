...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const call = await prisma.serviceCall.findFirst({
      ...
      include: {
        customer: true, device: { include: { brand: true } }, technician: true,
        partsUsed: { include: { part: { select: { id: true, name: true, category: true } } } },
        invoice: true,
      },
    });
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!session) return unauthorized();
  try {
    const call = await prisma.serviceCall.findFirst({ where: { id: id, organizationId: session.orgId } });
    ...
    const workingDaysUsed = completedAt ? Math.floor((completedAt.getTime() - new Date(call.receivedAt).getTime()) / 86400000) : undefined;
    const updated = await prisma.serviceCall.update({
      ...
      data: {
        ...
        completedAt, workingDaysUsed,
      },
    });
    if (parsed.data.status === "COMPLETED" || parsed.data.status === "CANNOT_REPAIR" || parsed.data.status === "CANCELED") {
      await prisma.device.update({
        where: { id: call.deviceId },
        data: { isUnderRepair: false, lastServiceAt: completedAt ?? undefined, totalServiceCount: { increment: parsed.data.status === "COMPLETED" ? 1 : 0 } },
      });
    }
