...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const device = await prisma.device.findFirst({
      ...
      include: {
        customer: true, brand: true,
        serviceCalls: { orderBy: { createdAt: "desc" }, take: 20, include: { technician: { select: { id: true, name: true } } } },
      },
    });
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await prisma.device.update({
      where: { id: id },
      data: { notes: body.notes ?? undefined, locationNote: body.locationNote ?? undefined, isUnderRepair: body.isUnderRepair ?? undefined },
    });
