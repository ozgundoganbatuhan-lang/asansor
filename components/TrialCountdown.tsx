...
export default function TrialCountdown() {
  ...
  useEffect(() => {
    fetch("/api/me")
      ...
      .then(d => { if (d?.org) setData({ trialEndsAt: d.org.trialEndsAt, plan: d.org.plan }); })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="trial-bar">
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <span style={{ fontSize: 14 }}>{days <= 2 ? "⚡" : "📅"}</span>
        <span style={{ fontSize: 13, fontWeight: 600 }}>
          {days === 0 ? "Deneme süreniz bugün bitiyor." : `Deneme sürenizde ${days} gün kaldı.`}
        </span>
        <span style={{ fontSize: 12, opacity: 0.55 }}>Tüm özellikler aktif.</span>
      </div>
