    ...
    <div style={{ minHeight: "100vh", background: "#f9fafb", display: "flex",
      ...
      <aside style={{
        ...
        ...(mob ? {
          ...
          boxShadow: open ? "4px 0 40px rgba(0,0,0,0.65)" : "none",
        } : {}),
        ...
        <div style={{ padding: "10px 12px", borderBottom: `1px solid ${SB_BORDER}`, flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, background: SB_HOVER,
            border: `1px solid ${SB_BORDER}`, borderRadius: 6, padding: "7px 10px", cursor: "pointer" }}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={SB_TEXT}
        ...
        <div style={{ margin: "10px 12px", flexShrink: 0 }}>
          <Link href="/technician" style={{ display: "flex", alignItems: "center", gap: 9,
                ...
                return (
                  <Link key={item.href} href={item.href}
                    style={{
                      ...
                      position: "relative" as const, transition: "background .1s,color .1s",
                    }}
                    onMouseEnter={e => { if (!active) { const el = e.currentTarget as HTMLElement; el.style.background = SB_HOVER; el.style.color = SB_TEXT_A; } }}
                    onMouseLeave={e => { if (!active) { const el = e.currentTarget as HTMLElement; el.style.background = "transparent"; el.style.color = SB_TEXT; } }}
                  >
                    {active && <span style={{ position: "absolute" as const, left: 0, top: "50%",
        ...
        <div style={{ flexShrink: 0, marginTop: "auto", borderTop: `1px solid ${SB_BORDER}` }}>
          <div style={{ margin: "12px 12px 4px", background: SB_HOVER, borderRadius: 10, padding: "10px 12px" }}>
            ...
            <Link href="/app/upgrade" style={{ display: "block", fontSize: 11, fontWeight: 600,
              color: "#3b82f6", marginTop: 8, textDecoration: "none" }}>
              Pro&apos;ya yükselt →
            </Link>
          </div>
          <div style={{ padding: "10px 12px", display: "flex", alignItems: "center", gap: 9 }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              <Link href="/app/settings" title="Ayarlar"
                style={{ color: SB_TEXT, padding: 4, borderRadius: 4,
              ...
              <button onClick={logout} title="Çıkış"
                style={{ background: "none", border: "none", cursor: "pointer",
                  color: SB_TEXT, padding: 4, borderRadius: 4, display: "inline-flex", alignItems: "center" }}>
      ...
      {/* MAIN — no topbar, pages render their own header */}
      <div style={{
        marginLeft: mob ? 0 : SW,
        flex: 1, width: mob ? "100%" : `calc(100% - ${SW}px)`,
        minHeight: "100vh", display: "flex", flexDirection: "column",
        paddingBottom: mob ? 64 : 0,
      }}>
        {mob && (
          <div style={{ height: 52, background: "#0d1117", display: "flex",
            alignItems: "center", paddingLeft: 16, flexShrink: 0,
            borderBottom: "1px solid #1e2d3d" }}>
            <button onClick={() => setOpen(v => !v)}
              style={{ background: "none", border: "none", cursor: "pointer",
                color: "#8b9ab0", display: "inline-flex", padding: 8 }}>
              <Ic d="M4 6h16M4 12h16M4 18h16" size={18} />
            </button>
            <span style={{ fontSize: 14, fontWeight: 700, color: "#3b82f6", marginLeft: 8 }}>Servisim</span>
          </div>
        )}
        <main className="page-animate" style={{
          flex: 1, padding: mob ? "16px" : "28px 32px",
        ...
        <nav style={{ position: "fixed", bottom: 0, left: 0, right: 0, height: 64,
          ...
          <button onClick={() => setOpen(true)}
            style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center",
              justifyContent: "center", gap: 3, fontSize: 9.5, fontWeight: 600, color: "#9ca3af",
