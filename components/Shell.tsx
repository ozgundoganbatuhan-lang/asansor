...
export default function Shell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="app-shell" style={{ minHeight: "100vh", background: "#f4f4f7", display: "flex" }}>
      {/* ══ SIDEBAR ══ */}
      <aside style={{
        ...
        {/* Logo */}
        <div style={{ padding: "20px 18px 14px", display: "flex", alignItems: "center", gap: 11, flexShrink: 0, borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
          <LogoMark size={32} />
          <div>
            <div style={{ fontSize: 15, fontWeight: 900, color: "#fff", letterSpacing: "-0.05em", lineHeight: 1 }}>
              Servi<span style={{ color: "#60a5fa" }}>sim</span>
            </div>
            <div style={{ fontSize: 9, color: "rgba(255,255,255,0.28)", marginTop: 3, fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase" }}>
              Asansör Operasyon
            </div>
          </div>
        </div>

        {/* Technician Portal CTA */}
        <div style={{ padding: "12px 12px 6px", flexShrink: 0 }}>
          <Link href="/technician" style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 11.5, fontWeight: 800, color: "#93c5fd", letterSpacing: "-0.01em" }}>Teknisyen Portalı</div>
              <div style={{ fontSize: 9.5, color: "rgba(255,255,255,0.35)", marginTop: 1 }}>Saha uygulaması</div>
            </div>
            <I d="M9 18l6-6-6-6" size={12} />
          </Link>
        </div>
        ...
        <nav style={{ padding: "6px 8px", flex: 1, overflowY: "auto" }}>
          {NAV.map(group => (
            <div key={group.group} style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: "rgba(255,255,255,0.18)", padding: "8px 10px 5px" }}>
                {group.group}
              </div>
              {group.items.map(item => {
                const isA = active(item.href);
                return (
                  <Link key={item.href} href={item.href} className="nav-link" data-active={isA} style={{
                    ...
                    background: isA ? "rgba(37,99,235,0.18)" : "transparent",
                    ...
                    <span style={{ width: 16, color: isA ? "#60a5fa" : "rgba(255,255,255,0.35)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <I d={item.icon} size={15} />
                    </span>
                    <span style={{ flex: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{item.label}</span>
                    {(item as { badge?: string }).badge && (
                      <span style={{ fontSize: 9.5, fontWeight: 700, background: "rgba(239,68,68,0.18)", color: "#f87171", border: "1px solid rgba(239,68,68,0.22)", padding: "1px 6px", borderRadius: 999 }}>
                        {(item as { badge?: string }).badge}
                      </span>
                    )}
                  </Link>
        ...
        <div style={{ padding: "10px 10px", borderTop: "1px solid rgba(255,255,255,0.05)", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "7px 8px", borderRadius: 9 }}>
            <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg,#1d4ed8,#7c3aed)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: "#fff", flexShrink: 0 }}>AK</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,0.85)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>Hesabım</div>
              <div style={{ fontSize: 9.5, color: "rgba(255,255,255,0.3)", marginTop: 1 }}>Yönetici</div>
            </div>
            <Link href="/app/settings" title="Ayarlar" style={{ padding: 5, color: "rgba(255,255,255,0.35)", display: "flex" }}>
            ...
            <button onClick={logout} title="Çıkış" style={{ background: "none", border: "none", cursor: "pointer", padding: 5, color: "rgba(255,255,255,0.35)", display: "flex" }}>
      ...
      {/* Mobile overlay */}
      {mobile && open && (
        <div onClick={() => setOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", zIndex: 39, backdropFilter: "blur(4px)" }} />
      )}

      {/* ══ APP BODY ══ */}
      <div style={{ marginLeft: mobile ? 0 : SIDEBAR_W, flex: 1, minHeight: "100vh", paddingBottom: mobile ? 64 : 0 }}>
        {/* Topbar */}
        <header style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            {mobile && (
              <button onClick={() => setOpen(v => !v)} style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 36, height: 36, borderRadius: 9, border: "1px solid rgba(0,0,0,0.07)", background: "#fff", cursor: "pointer", color: "#0d1117", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <button style={{ width: 36, height: 36, borderRadius: 9, background: "#fff", border: "1px solid rgba(0,0,0,0.06)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", position: "relative" }}>
              ...
              <div style={{ position: "absolute", top: 7, right: 7, width: 7, height: 7, borderRadius: "50%", background: "#ef4444", border: "2px solid #f4f4f7" }} />
            </button>
        ...
        <main style={{ padding: mobile ? "16px" : "28px 32px" }}>{children}</main>
      </div>
      ...
      {mobile && (
        <nav style={{ position: "fixed", bottom: 0, left: 0, right: 0, height: 64, background: "#fff", borderTop: "1px solid rgba(0,0,0,0.06)", zIndex: 40, padding: "0 8px", display: "flex", alignItems: "center", justifyContent: "space-around", boxShadow: "0 -4px 20px rgba(0,0,0,0.05)" }}>
          {MOBILE_NAV.map(item => {
            const isA = active(item.href);
            return (
              <Link key={item.href} href={item.href} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3, padding: "8px 4px", borderRadius: 9, color: isA ? "#2563eb" : "#a1a1aa", fontSize: 9.5, fontWeight: 600, textDecoration: "none" }}>
                <I d={item.icon} size={19} />
                <span>{item.label.split(" ")[0]}</span>
              </Link>
            );
          })}
          <button onClick={() => setOpen(true)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3, fontSize: 9.5, fontWeight: 600, color: "#a1a1aa", background: "none", border: "none", cursor: "pointer", padding: "8px 4px" }}>
            <I d="M4 6h16M4 12h16M4 18h16" size={19} />
            <span>Daha</span>
          </button>
        </nav>
      )}
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .nav-link:hover[data-active="false"] { background: rgba(255,255,255,0.05) !important; color: rgba(255,255,255,0.82) !important; }
        .app-shell ::-webkit-scrollbar { width: 5px; height: 5px; }
        .app-shell ::-webkit-scrollbar-track { background: transparent; }
        .app-shell ::-webkit-scrollbar-thumb { background: rgba(0,0,0,0.1); border-radius: 999px; }
        .app-shell aside ::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.05); }
      `}} />
    </div>
  );
}
