    ...
    <div style={{ background:"#fff", minHeight:"100vh", fontFamily:"'Inter',-apple-system,system-ui,sans-serif", color:"#0f172a" }}>
      ...
      <section style={{ background:"linear-gradient(180deg,#f9fafb 0%,#fff 100%)", borderBottom:"1px solid #e5e7eb", padding:"48px 24px 40px" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          ...
          <h1 style={{ fontSize:"clamp(26px,4vw,44px)", fontWeight:900, letterSpacing:"-0.05em", lineHeight:1.05, color:"#0f172a", margin:"0 0 10px", maxWidth:640 }}>Asansör servis operasyon blogu</h1>
      ...
      <main style={{ maxWidth:1200, margin:"0 auto", padding:"44px 24px 68px" }}>
        ...
        {featured && (
          <div style={{ marginBottom:44 }}>
            <div style={{ fontSize:10, fontWeight:700, color:"#94a3b8", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:12 }}>Öne çıkan yazı</div>
            <Link href={`/blog/${featured.slug}`} className="featured-link" style={{ textDecoration:"none", display:"grid", gridTemplateColumns:"1fr 1.1fr", background:"#fff", borderRadius:16, border:"1px solid #e5e7eb", overflow:"hidden", boxShadow:"0 3px 16px rgba(0,0,0,0.06)" }}>
              ...
              <div style={{ padding:"28px 32px", display:"flex", flexDirection:"column", justifyContent:"center" }}>
                <div style={{ display:"flex", gap:7, marginBottom:10, alignItems:"center" }}>
                  ...
                  <span style={{ fontSize:11, color:"#94a3b8" }}>{featured.readingMinutes} dk okuma</span>
                </div>
                <h2 style={{ fontSize:"clamp(16px,2vw,24px)", fontWeight:800, color:"#0f172a", letterSpacing:"-0.03em", lineHeight:1.2, margin:"0 0 9px" }}>{featured.title}</h2>
            ...
            <Link key={post.slug} href={`/blog/${post.slug}`} className="blog-card" style={{ textDecoration:"none", background:"#fff", borderRadius:13, border:"1px solid #e5e7eb", overflow:"hidden", display:"flex", flexDirection:"column" }}>
              ...
              <div style={{ padding:"15px 17px", flex:1, display:"flex", flexDirection:"column" }}>
                <div style={{ display:"flex", gap:5, marginBottom:7 }}>
                  ...
                  <span style={{ fontSize:9.5, color:"#94a3b8" }}>{post.readingMinutes} dk</span>
                </div>
                <h2 style={{ fontSize:13, fontWeight:700, color:"#0f172a", letterSpacing:"-0.02em", lineHeight:1.35, margin:"0 0 6px" }}>{post.title}</h2>
      ...
      <section style={{ background:"#0f172a", padding:"48px 24px" }}>
        <div style={{ maxWidth:560, margin:"0 auto", textAlign:"center" }}>
          ...
          <div style={{ display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap" }}>
            <Link href="/auth/register" style={{ background:"#fff", color:"#0f172a", fontSize:13, fontWeight:800, padding:"10px 20px", borderRadius:8, textDecoration:"none" }}>Ücretsiz başla →</Link>
      ...
      <footer style={{ background:"#0f172a", borderTop:"1px solid rgba(255,255,255,0.05)", padding:"20px", textAlign:"center" }}>
