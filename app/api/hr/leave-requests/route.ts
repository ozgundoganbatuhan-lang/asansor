...
export async function GET(req: NextRequest) {
  ...
  /* Store on a JSON field or a real table depending on schema;
     here we use a generic "leave_request" table name that may need migration */
  try {
    ...
    /* Table may not exist yet — return empty so UI doesn't crash */
...
export async function POST(req: NextRequest) {
  ...
  const { leaveType, startDate, endDate, note, days, docUrls, technicianId } = parsed.data;
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    const item = await (prisma as any).leaveRequest.update({
