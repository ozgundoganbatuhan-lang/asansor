...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const partsTotal = call.partsUsed.filter((p: { isWarrantyCovered: boolean; unitPrice: number; quantity: number }) => !p.isWarrantyCovered).reduce((a: number, p: { isWarrantyCovered: boolean; unitPrice: number; quantity: number }) => a + p.unitPrice * p.quantity, 0);
