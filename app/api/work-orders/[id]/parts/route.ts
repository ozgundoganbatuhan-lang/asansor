...
const schema = z.object({ partId: z.string().min(1), quantity: z.coerce.number().int().min(1).default(1) });
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!parsed.success) return NextResponse.json({ error: parsed.error.issues[0]?.message ?? "Geçersiz" }, { status: 400 });
  const part = await prisma.part.findFirst({ where: { id: parsed.data.partId, organizationId: session.orgId } });
  if (!part) return NextResponse.json({ error: "Parça bulunamadı." }, { status: 404 });
  const usage = await prisma.partUsage.create({
    data: { workOrderId: id, partId: parsed.data.partId, quantity: parsed.data.quantity },
    include: { part: true },
  });
