      ...
      <div style={{ width:"100%", maxWidth:460 }}>
        <div style={{ textAlign:"center", marginBottom:32 }}>
          <Link href="/" style={{ textDecoration:"none" }}>
            <div style={{ fontSize:28, fontWeight:900, color:"white", letterSpacing:-1 }}>servisim<span style={{ color:"#3b82f6" }}>.</span></div>
          </Link>
        ...
        <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:20, padding:36 }}>
          <h1 style={{ color:"white", fontSize:22, fontWeight:800, marginBottom:6 }}>Hesap Oluşturun</h1>
          <p style={{ color:"#64748b", fontSize:14, marginBottom:28 }}>14 gün ücretsiz, kredi kartı gerekmez.</p>

          <form onSubmit={onSubmit} style={{ display:"flex", flexDirection:"column", gap:16 }}>
            <Field label="Firma Adı" required>
              <input style={inp} value={organizationName}
                onChange={e => { setOrganizationName(e.target.value); if (!organizationSlug) setOrganizationSlug(slugify(e.target.value)); }}
                placeholder="Örnek Asansör Servisi" required />
            </Field>

            <Field label="Firma Kısa Adı (URL)" hint="Sadece a-z, 0-9 ve - karakterleri" required>
              <div style={{ position:"relative" }}>
                <span style={{ position:"absolute", left:12, top:"50%", transform:"translateY(-50%)", color:"#475569", fontSize:13, pointerEvents:"none" }}>servisim/</span>
                <input style={{ ...inp, paddingLeft:90 }} value={organizationSlug}
                  onChange={e => setOrganizationSlug(slugify(e.target.value))}
                  placeholder="ornek-servis" required />
              </div>
            </Field>
            ...
            <Field label="Ad Soyad">
              <input style={inp} value={name} onChange={e => setName(e.target.value)} placeholder="Ali Yılmaz" />
            </Field>
            ...
            <Field label="Şifre" required>
              <input style={inp} value={password} onChange={e => setPassword(e.target.value)} type="password" placeholder="En az 8 karakter" minLength={8} required />
            </Field>
            ...
            <button type="submit" disabled={loading} style={{
              ...
              opacity: loading ? 0.7 : 1, marginTop:4, boxShadow:"0 4px 20px rgba(37,99,235,0.3)",
            }}>
        ...
        <div style={{ textAlign:"center", marginTop:20, color:"#475569", fontSize:14 }}>
          ...
          <Link href="/auth/login" style={{ color:"#60a5fa", fontWeight:700, textDecoration:"none" }}>Giriş Yapın</Link>
        </div>
      </div>
    </div>
  );
