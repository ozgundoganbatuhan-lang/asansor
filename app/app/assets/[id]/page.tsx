...
export default function AssetDetailPage() {
  ...
  if(!asset) return <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:300,gap:12,color:"#64748b"}}><div style={{width:24,height:24,border:"3px solid #e2e8f0",borderTopColor:"#7c3aed",borderRadius:"50%",animation:"spin 0.7s linear infinite"}}/>Yükleniyor...</div>;

  const riskColor = asset.riskScore!=null ? (asset.riskScore>=8?"#dc2626":asset.riskScore>=5?"#d97706":"#22c55e") : "#94a3b8";

  return (
    <div style={{maxWidth:1000,margin:"0 auto"}}>
      {/* Breadcrumb */}
      <div style={{display:"flex",alignItems:"center",gap:8,fontSize:13,color:"#64748b",marginBottom:20}}>
        <Link href="/app/assets" style={{color:"#64748b",textDecoration:"none",fontWeight:600}}>← Asansörler</Link>
        <span>/</span>
        <span style={{color:"#0f172a",fontWeight:700}}>{asset.name}</span>
      </div>

      {/* Hero */}
      <div style={{background:"linear-gradient(135deg,#0d1117,#1a1d2e)",borderRadius:18,padding:"28px 32px",color:"#fff",marginBottom:20,position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:20,flexWrap:"wrap"}}>
          <div>
            ...
            <div style={{display:"flex",gap:10,marginTop:16,flexWrap:"wrap"}}>
              ...
              {latestInsp && <span style={{background:`${LABEL_DOT[latestInsp.label]??"#94a3b8"}22`,border:`1px solid ${LABEL_DOT[latestInsp.label]??"#94a3b8"}44`,color:LABEL_DOT[latestInsp.label]??"#94a3b8",borderRadius:999,padding:"4px 12px",fontSize:12,fontWeight:700}}>● {latestInsp.label} etiket</span>}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 320px",gap:20,alignItems:"start"}}>
        ...
        <div>
          ...
          <div style={{display:"flex",gap:4,background:"#f1f5f9",borderRadius:14,padding:4,marginBottom:16}}>
            {([{k:"timeline",l:"📋 Servis Geçmişi"},{k:"inspections",l:"🔍 Kontroller"},{k:"plans",l:"🔧 Bakım Planları"}] as const).map(t=>(
              <button key={t.k} onClick={()=>setTab(t.k)} style={{flex:1,padding:"9px",borderRadius:10,border:"none",cursor:"pointer",fontSize:12.5,fontWeight:700,transition:"all 0.15s",background:tab===t.k?"#fff":"transparent",color:tab===t.k?"#0f172a":"#64748b",boxShadow:tab===t.k?"0 1px 4px rgba(0,0,0,0.08)":"none",fontFamily:"inherit"}}>
                {t.l}
              </button>
            ))}
          </div>
          ...
          {tab==="timeline" && (
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              {asset.workOrders.length===0 ? (
                <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:16,padding:"40px 24px",textAlign:"center"}}><div style={{fontSize:40,marginBottom:10}}>📋</div><div style={{fontSize:14,fontWeight:700,color:"#475569"}}>Servis kaydı yok</div></div>
              ) : asset.workOrders.map(wo=>{
                const cfg=S_CFG[wo.status]??{bg:"#f1f5f9",color:"#475569",dot:"#94a3b8"};
                return (
                  <Link key={wo.id} href={`/app/work-orders/${wo.id}`} style={{display:"block",background:"#fff",border:"1px solid #e2e8f0",borderRadius:16,padding:"16px 20px",textDecoration:"none",borderLeft:`4px solid ${cfg.dot}`,transition:"all 0.12s"}}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,marginBottom:8}}>
                      <div style={{display:"flex",alignItems:"center",gap:10}}>
                        <span style={{fontFamily:"monospace",fontSize:11.5,fontWeight:700,color:"#94a3b8"}}>{wo.code}</span>
                    ...
                    <div style={{fontSize:13.5,fontWeight:700,color:"#0f172a",marginBottom:2}}>{TYPE_L[wo.type]??wo.type}</div>
                    {wo.technician && <div style={{fontSize:12,color:"#64748b"}}>👤 {wo.technician.name}</div>}
                  </Link>
                );
              })}
            </div>
          ...
          {tab==="inspections" && (
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {asset.inspections.length===0 ? (
                <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:16,padding:"40px 24px",textAlign:"center"}}><div style={{fontSize:40,marginBottom:10}}>🔍</div><div style={{fontSize:14,fontWeight:700,color:"#475569"}}>Kontrol kaydı yok</div></div>
              ) : asset.inspections.map(ins=>{
                const dot=LABEL_DOT[ins.label]??"#94a3b8";
                return (
                  <div key={ins.id} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:16,padding:"16px 20px",borderLeft:`4px solid ${dot}`}}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
                      <div>
                        <div style={{fontSize:14,fontWeight:800,color:"#0f172a"}}>{new Date(ins.inspectionDate).toLocaleDateString("tr-TR")}</div>
                        <div style={{fontSize:12,color:"#64748b",marginTop:2}}>{ins.result}</div>
                      </div>
                    ...
                    {ins.nextDueDate && <div style={{fontSize:12,color:"#64748b",marginTop:8}}>Sonraki: {new Date(ins.nextDueDate).toLocaleDateString("tr-TR")}</div>}
                  </div>
                );
              })}
            </div>
          ...
          {tab==="plans" && (
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {asset.maintenancePlans.length===0 ? (
                <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:16,padding:"40px 24px",textAlign:"center"}}><div style={{fontSize:40,marginBottom:10}}>🔧</div><div style={{fontSize:14,fontWeight:700,color:"#475569"}}>Bakım planı yok</div></div>
              ) : asset.maintenancePlans.map(plan=>{
                const due=new Date(plan.nextDueAt);
                const overdue=due<new Date();
                return (
                  <div key={plan.id} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:16,padding:"16px 20px"}}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
                      <div>
                        <div style={{fontSize:14,fontWeight:800,color:"#0f172a"}}>{plan.name||PLAN_L[plan.planType]||plan.planType}</div>
                        <div style={{fontSize:12,color:"#64748b",marginTop:2}}>Sonraki: {due.toLocaleDateString("tr-TR")}</div>
                      </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:18,padding:"20px"}}>
            <div style={{fontSize:13,fontWeight:800,color:"#0f172a",marginBottom:14}}>⚙️ Ekipman Bilgileri</div>
            {[
          ...
          {/* Müşteri */}
          <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:18,padding:"20px"}}>
            <div style={{fontSize:13,fontWeight:800,color:"#0f172a",marginBottom:14}}>👤 Müşteri</div>
            <div style={{fontSize:14,fontWeight:700,color:"#0f172a",marginBottom:4}}>{asset.customer.name}</div>
            {asset.customer.address && <div style={{fontSize:12.5,color:"#64748b",marginBottom:10,lineHeight:1.5}}>{asset.customer.address}</div>}
