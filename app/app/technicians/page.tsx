...
const STATUS_CFG: Record<string, { bg: string; color: string; dot: string; border: string }> = {
  müsait:  { bg: "#f0fdf4", color: "#15803d", dot: "#22c55e", border: "#bbf7d0" },
  saha:    { bg: "#fffbeb", color: "#b45309", dot: "#f59e0b", border: "#fcd34d" },
  ofis:    { bg: "#eff6ff", color: "#1d4ed8", dot: "#3b82f6", border: "#bfdbfe" },
  izinli:  { bg: "#f5f3ff", color: "#6d28d9", dot: "#8b5cf6", border: "#ddd6fe" },
  default: { bg: "#f1f5f9", color: "#475569", dot: "#94a3b8", border: "#e2e8f0" },
};
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: "#94a3b8", marginBottom: 6 }}>Ekip Yönetimi</div>
          <h1 style={{ fontSize: 26, fontWeight: 900, letterSpacing: "-0.05em", color: "#0f172a", margin: 0 }}>Teknisyenler</h1>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <Link href="/technician" target="_blank" style={{
            ...
            color: "#1d4ed8", borderRadius: 10, padding: "9px 15px", fontSize: 13, fontWeight: 700, textDecoration: "none",
          }}>
            🛗 Teknisyen Portalı
          </Link>
          <button onClick={() => setShowForm(v => !v)} style={{
            display: "inline-flex", alignItems: "center", gap: 7,
            background: showForm ? "#fff" : "#0f172a", color: showForm ? "#0f172a" : "#fff",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 10 }}>
        {[
          { label: "Toplam",  val: items.length, color: "#0f172a", bg: "#f1f5f9" },
      ...
      {showForm && (
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 16, padding: "22px 24px", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
          <div style={{ fontSize: 14, fontWeight: 800, color: "#0f172a", marginBottom: 18 }}>Yeni Teknisyen</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 14, marginBottom: 16 }}>
            {([ { label: "Ad Soyad *", field: "name" }, { label: "Kısa", field: "initials", placeholder: "AY" }, { label: "Telefon", field: "phone", type: "tel" }, { label: "Bölge", field: "zone", placeholder: "Kadıköy" }, { label: "Sertifika", field: "certification" }, { label: "Durum", field: "status" } ] as const).map(f => (
              <div key={f.field}>
                <label style={{ display: "block", fontSize: 11, fontWeight: 700, letterSpacing: "0.06em", color: "#64748b", marginBottom: 7 }}>{f.label}</label>
                {f.field === "status" ? (
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => setShowForm(false)} style={{ background: "#f1f5f9", border: "none", borderRadius: 10, padding: "10px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer", color: "#475569", fontFamily: "inherit" }}>İptal</button>
            <button onClick={create} disabled={!form.name.trim() || saving} style={{ background: "#2563eb", color: "#fff", border: "none", borderRadius: 10, padding: "10px 20px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 3px 12px rgba(37,99,235,0.28)", opacity: saving ? 0.7 : 1 }}>
      ...
      {loading ? (
        ...
        <div style={{ background: "#fff", border: "1.5px dashed #e2e8f0", borderRadius: 16, padding: "44px 24px", textAlign: "center" }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 800, color: "#0f172a", marginBottom: 6 }}>{q ? "Sonuç bulunamadı" : "Henüz teknisyen yok"}</div>
          <div style={{ fontSize: 13, color: "#64748b" }}>{q ? "Farklı bir arama deneyin." : "Ekibinizin ilk teknisyenini ekleyin."}</div>
        </div>
      ) : (
              ...
              <div key={t.id} className="tech-card" style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 16, padding: "20px", boxShadow: "0 2px 8px rgba(0,0,0,0.04)", transition: "all .18s" }}>
                <div style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 16 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14.5, fontWeight: 800, color: "#0f172a", letterSpacing: "-0.03em", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.name}</div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                  {t.phone && (
                    <a href={`tel:${t.phone}`} style={{ display: "flex", alignItems: "center", gap: 9, fontSize: 13, color: "#2563eb", textDecoration: "none", fontWeight: 600 }}>
                      <span style={{ color: "#94a3b8" }}><I d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></span>
                      {t.phone}
                    </a>
                  )}
                  {t.zone && (
                    <div style={{ display: "flex", alignItems: "center", gap: 9, fontSize: 13, color: "#475569" }}>
                      <span style={{ color: "#94a3b8" }}><I d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></span>
                      {t.zone}
                    </div>
                  )}
                  {t.certification && (
                    <div style={{ display: "flex", alignItems: "center", gap: 9, fontSize: 13, color: "#475569" }}>
                      <span style={{ color: "#94a3b8" }}><I d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" /></span>
                      {t.certification}
                    </div>
                  )}
                </div>
                <div style={{ marginTop: 16, paddingTop: 14, borderTop: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <span style={{ fontSize: 11, color: "#94a3b8" }}>Katılım: {new Date(t.createdAt).toLocaleDateString("tr-TR")}</span>
                  <Link href={`/technician?tech=${t.id}`} target="_blank" style={{ fontSize: 11.5, fontWeight: 700, color: "#2563eb", textDecoration: "none", background: "#eff6ff", borderRadius: 7, padding: "4px 10px" }}>
                    Portala git →
                  </Link>
                </div>
              </div>
