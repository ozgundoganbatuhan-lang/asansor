...
const fb = (e: React.FocusEvent<HTMLInputElement>) => e.target.style.borderColor="#e4e4e7";
const LBL = ({c}:{c:string}) => <label style={{display:"block",fontSize:11,fontWeight:700,letterSpacing:"0.12em",textTransform:"uppercase",color:"#a1a1aa",marginBottom:8}}>{c}</label>;
  ...
  return (
    <div style={{maxWidth:960,margin:"0 auto"}}>
      <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",gap:16,marginBottom:24,flexWrap:"wrap"}}>
        <div>
          <div style={{fontSize:11,fontWeight:700,letterSpacing:"0.16em",textTransform:"uppercase",color:"#a1a1aa",marginBottom:6}}>Depo & Lojistik</div>
          <h1 style={{fontSize:28,fontWeight:900,letterSpacing:"-0.04em",color:"#0a0a0f",margin:0}}>Stok</h1>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12,marginBottom:20}}>
        {[
          {l:"Toplam Çeşit",v:parts.length,c:"#0a0a0f",bg:"#f4f4f6"},
          ...
          <div key={s.l} style={{background:s.bg,borderRadius:14,padding:"14px 18px"}}>
            ...
            <div style={{fontSize:11.5,color:"#71717a",marginTop:3,fontWeight:600}}>{s.l}</div>
          </div>
        ))}
      </div>
      ...
      {/* Add form */}
      {showForm && (
        <div style={{background:"#fff",border:"1px solid #e4e4e7",borderRadius:20,padding:"24px",marginBottom:20}}>
          <div style={{fontSize:14,fontWeight:800,color:"#0a0a0f",marginBottom:18}}>➕ Yeni Parça</div>
          ...
          <div style={{display:"flex",justifyContent:"flex-end",gap:10}}>
            <button onClick={()=>setShowForm(false)} style={{background:"#f4f4f6",border:"none",borderRadius:10,padding:"10px 18px",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",color:"#52525b"}}>İptal</button>
            <button onClick={add} disabled={!form.name.trim()||saving} style={{background:form.name.trim()?"#2563eb":"#e4e4e7",color:form.name.trim()?"#fff":"#a1a1aa",border:"none",borderRadius:10,padding:"10px 20px",fontSize:13,fontWeight:700,cursor:form.name.trim()?"pointer":"not-allowed",fontFamily:"inherit"}}>
      ...
      <div style={{background:"#fff",border:"1px solid #e4e4e7",borderRadius:20,overflow:"hidden"}}>
        <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 80px 80px 100px 1.5fr",padding:"12px 20px",background:"#f9f9fb",borderBottom:"1px solid #e4e4e7"}}>
          {["Parça","Kategori","Stok","Min","Fiyat","Tedarikçi"].map(h=>(
            <div key={h} style={{fontSize:10.5,fontWeight:700,letterSpacing:"0.12em",textTransform:"uppercase",color:"#a1a1aa"}}>{h}</div>
          ))}
        </div>
        {loading ? (
          <div style={{padding:"40px 20px",textAlign:"center",color:"#a1a1aa"}}>Yükleniyor...</div>
        ) : filtered.length===0 ? (
          <div style={{padding:"40px 20px",textAlign:"center"}}>
            <div style={{fontSize:40,marginBottom:10}}>📦</div>
            <div style={{fontSize:15,fontWeight:700,color:"#0a0a0f"}}>Stok kaydı yok</div>
          </div>
        ) : filtered.map((p,i)=>{
          const low = p.stock<p.minStock;
          return (
            <div key={p.id} style={{display:"grid",gridTemplateColumns:"2fr 1fr 80px 80px 100px 1.5fr",padding:"14px 20px",borderBottom:i<filtered.length-1?"1px solid #f4f4f5":"none",alignItems:"center",transition:"background 0.1s"}}>
              <div style={{fontWeight:700,color:"#0a0a0f",fontSize:13}}>{p.name}</div>
              <div style={{fontSize:13,color:"#52525b"}}>{p.category??"—"}</div>
              <div>
              ...
              <div style={{fontSize:13,color:"#71717a"}}>{p.minStock}</div>
              <div style={{fontSize:13,color:"#52525b"}}>{p.price?`₺${p.price.toLocaleString("tr-TR")}`:"—"}</div>
              <div style={{fontSize:13,color:"#71717a"}}>{p.supplier??"—"}</div>
            </div>
          );
        })}
      </div>
