  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",alignItems:"flex-end",justifyContent:"space-between",gap:14}}>
        <div>
          <div style={{fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.18em",color:"#94a3b8",marginBottom:6}}>Operasyon</div>
          <h1 style={{fontSize:26,fontWeight:900,letterSpacing:"-0.05em",color:"#0f172a",lineHeight:1.05,margin:0}}>İş Emirleri</h1>
          <p style={{marginTop:6,fontSize:13.5,lineHeight:1.65,color:"#64748b",maxWidth:540,margin:"6px 0 0"}}>Planla, ata, sahayı görünür kıl. Operasyonun ritmi tek listede.</p>
        </div>
      ...
      {err&&<div style={{background:"#fef2f2",border:"1px solid #fecaca",color:"#b91c1c",padding:"12px 16px",borderRadius:12,fontSize:13}}>{err}</div>}

      {/* KPIs */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12}}>
        {[{l:"Toplam",v:rows.length,c:"#0f172a",bg:"#f1f5f9"},{l:"Acil",v:counts.URGENT,c:"#dc2626",bg:"#fef2f2"},{l:"Devam Eden",v:counts.IN_PROGRESS,c:"#d97706",bg:"#fffbeb"},{l:"Tamamlanan",v:counts.DONE,c:"#059669",bg:"#f0fdf4"}].map((k,i)=>(
      ...
      {showForm&&(
        <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:16,padding:"22px 24px",boxShadow:"0 2px 8px rgba(15,23,42,.04)"}}>
          <div style={{fontSize:14,fontWeight:800,color:"#0f172a",marginBottom:18,letterSpacing:"-0.03em"}}>Yeni İş Emri</div>
          <form onSubmit={create}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:14}}>
              {[
                ...
                <div key={i} style={{display:"flex",flexDirection:"column",gap:7}}>
                  <label style={{fontSize:11,fontWeight:700,letterSpacing:"0.06em",color:"#64748b"}}>{f.l}</label>
                  {f.el}
                </div>
              ))}
            </div>
            <div style={{marginTop:14,display:"flex",flexDirection:"column",gap:7}}>
              <label style={{fontSize:11,fontWeight:700,letterSpacing:"0.06em",color:"#64748b"}}>Not / açıklama</label>
            ...
            {cId&&filteredA.length===0&&<div style={{marginTop:12,background:"#fffbeb",border:"1px solid #fcd34d",borderRadius:10,padding:"10px 14px",fontSize:12.5,color:"#92400e"}}>
            ...
            <div style={{display:"flex",gap:8,marginTop:18,flexWrap:"wrap"}}>
              <button type="submit" style={{background:"#2563eb",color:"#fff",fontSize:13,fontWeight:700,padding:"10px 22px",borderRadius:10,border:"none",cursor:"pointer",fontFamily:"inherit",boxShadow:"0 3px 12px rgba(37,99,235,.28)"}}>İş Emrini Oluştur</button>
              <button type="button" onClick={()=>setShowForm(false)} style={{background:"#fff",color:"#0f172a",fontSize:13,fontWeight:600,padding:"10px 20px",borderRadius:10,border:"1.5px solid #e2e8f0",cursor:"pointer",fontFamily:"inherit"}}>İptal</button>
            </div>
          </form>
        </div>
      )}
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          {FILTERS.map(f=>(
            <button key={f} onClick={()=>setFS(f)} style={{padding:"7px 14px",borderRadius:999,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",
              background:fs===f?"#0f172a":"#fff",color:fs===f?"#fff":"#475569",
              border:`1.5px solid ${fs===f?"#0f172a":"#e2e8f0"}`,transition:"all .12s"}}>
      ...
      {/* List */}
      {loading?(
        <div style={{minHeight:200,display:"flex",alignItems:"center",justifyContent:"center",gap:12,color:"#64748b"}}>
        ...
        <div style={{background:"#fff",border:"1.5px dashed #e2e8f0",borderRadius:16,padding:44,textAlign:"center"}}>
          ...
          <div style={{fontSize:15,fontWeight:800,color:"#0f172a",marginBottom:6}}>{q||fs!=="ALL"?"Eşleşen iş emri bulunamadı":"Henüz iş emri yok"}</div>
          <div style={{fontSize:13,color:"#64748b"}}>{q||fs!=="ALL"?"Farklı filtre veya arama deneyin.":"İlk iş emrini oluşturarak başlayın."}</div>
        </div>
      ):(
        <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:16,overflow:"hidden",boxShadow:"0 2px 8px rgba(15,23,42,.04)"}}>
          <div style={{display:"grid",gridTemplateColumns:"auto 1fr 160px 90px 100px",alignItems:"center",gap:0,padding:"11px 22px",background:"#f8fafc",borderBottom:"1px solid #f1f5f9"}}>
              ...
              <Link key={o.id} href={`/app/work-orders/${o.id}`} className="wo-r" style={{
                ...
                <div style={{minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:2}}>
                    <span style={{fontFamily:"ui-monospace,monospace",fontSize:11.5,fontWeight:700,color:"#2563eb",flexShrink:0}}>{o.code}</span>
                    <span style={{fontSize:13.5,fontWeight:700,color:"#0f172a",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{o.asset?.name??"Asansör atanmadı"}</span>
                  </div>
                  <div style={{fontSize:11.5,color:"#64748b"}}>{o.customer.name} · {TYPE_L[o.type]??o.type}</div>
                </div>
                <div style={{fontSize:12.5,color:o.technician?"#334155":"#94a3b8",fontWeight:o.technician?600:400,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{o.technician?.name??"Atanmadı"}</div>
                <div style={{fontSize:11.5,color:"#94a3b8"}}>{new Date(o.createdAt).toLocaleDateString("tr-TR",{day:"2-digit",month:"2-digit"})}</div>
